...
fn invalidate_range_clears_cached_block() {
    ...
    ctx.pc = CODE_BASE;
    let mut mem = CodeMem {
        bytes: build_code(&[movz_x0(0x111), BRK_0]),
        base: CODE_BASE,
    };

    jit.run(&mut ctx, &mut mem).unwrap();
    assert!(
        jit.cache.lookup(CODE_BASE).is_some(),
        "block should be cached after first run"
    );

    jit.invalidate_range(CODE_BASE, 4);
    assert!(
        jit.cache.lookup(CODE_BASE).is_none(),
...
fn invalidate_range_forces_recompile_after_rewrite() {
    ...
    assert_eq!(
        ctx.x[0], 0x111,
        "stale cache still returns the old constant"
    );
    ...
    assert_eq!(
        ctx.x[0], 0x222,
        "after invalidate, recompile picks up new bytes"
    );
}
