...
fn cntfrq_reads_context_field() {
    ...
    assert_eq!(
        ctx.x[0], 19_200_000,
        "CNTFRQ_EL0 default should be the Switch rate (19.2 MHz)"
    );
}
...
fn cntfrq_honors_embedder_override() {
    ...
    run_with(code, &mut ctx);
    assert_eq!(
        ctx.x[0], 1_000_000_000,
        "embedder-set CNTFRQ_EL0 should be visible to MRS"
    );
}
...
fn cntpct_honors_embedder_callback() {
    ...
    ctx.pc = CODE_BASE;
    unsafe {
        TEST_COUNTER = 0xCAFE_F00D_0000_0000;
    }
    run_with(code, &mut ctx);
    assert_eq!(
        ctx.x[0], 0xCAFE_F00D_0000_0000,
        "first MRS should see initial counter"
    );
    assert_eq!(
        ctx.x[1], 0xCAFE_F00D_0000_0001,
        "second MRS should see incremented counter"
    );
}
