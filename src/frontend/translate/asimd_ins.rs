...
fn decode_imm5(imm5: u32) -> Option<(u32, u32)> {
    if imm5 & 0b00001 != 0 {
        return Some((0, (imm5 >> 1) & 0xF));
    }
    if imm5 & 0b00010 != 0 {
        return Some((1, (imm5 >> 2) & 0x7));
    }
    if imm5 & 0b00100 != 0 {
        return Some((2, (imm5 >> 3) & 0x3));
    }
    if imm5 & 0b01000 != 0 {
        return Some((3, (imm5 >> 4) & 0x1));
    }
    None
}
...
fn translate_dup_from_gpr(em: &mut IrEmitter<'_>, raw: u32) -> Result<InstStatus> {
    ...
    let (lane_log2, _) = decode_imm5(imm5).ok_or(Error::Decode {
        pc: em.current_pc,
        opcode: raw,
    })?;
    let gpr_size = if lane_log2 == 3 {
        RegSize::X
    } else {
        RegSize::W
    };
...
fn translate_dup_from_element(em: &mut IrEmitter<'_>, raw: u32) -> Result<InstStatus> {
    ...
    let rn = bits(raw, 5, 5) as u8;
    let rd = bits(raw, 0, 5) as u8;
    let (lane_log2, src_lane) = decode_imm5(imm5).ok_or(Error::Decode {
        pc: em.current_pc,
        opcode: raw,
    })?;
...
fn translate_umov(em: &mut IrEmitter<'_>, raw: u32) -> Result<InstStatus> {
    ...
    let rn = bits(raw, 5, 5) as u8;
    let rd = bits(raw, 0, 5) as u8;
    let (lane_log2, lane) = decode_imm5(imm5).ok_or(Error::Decode {
        pc: em.current_pc,
        opcode: raw,
    })?;
    ...
    match lane_log2 {
        0 => {
            let v = em.vec_extract_u8(src_q, lane);
            em.set_w(rd, v);
        }
        1 => {
            let v = em.vec_extract_u16(src_q, lane);
            em.set_w(rd, v);
        }
        2 => {
            let v = em.vec_extract_u32(src_q, lane);
            em.set_w(rd, v);
        }
        3 => {
            if !q {
...
fn translate_ins_from_element(em: &mut IrEmitter<'_>, raw: u32) -> Result<InstStatus> {
    ...
    let rn = bits(raw, 5, 5) as u8;
    let rd = bits(raw, 0, 5) as u8;
    let (lane_log2, dst_lane) = decode_imm5(imm5).ok_or(Error::Decode {
        pc: em.current_pc,
        opcode: raw,
    })?;
