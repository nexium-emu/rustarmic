...
fn run_unicorn(code: &[u8], init: RegState) -> RegState {
    let mut emu = Unicorn::new(Arch::ARM64, Mode::LITTLE_ENDIAN).expect("unicorn init failed");
    ...
    emu.reg_write(RegisterARM64::NZCV, (init.nzcv as u64) << 28)
        .unwrap();
    for i in 0..32 {
        let mut buf = [0u8; 16];
