use disarm64::decoder::CONDCMP_REG;
...
pub fn translate(em: &mut IrEmitter<'_>, insn: CONDCMP_REG) -> Result<InstStatus> {
    use CONDCMP_REG::*;
    let (raw, is_sub) = match insn {
        CCMP_Rn_Rm_NZCV_COND(i) => (i.0, true),
        CCMN_Rn_Rm_NZCV_COND(i) => (i.0, false),
    };
    let sf = bit(raw, 31);
